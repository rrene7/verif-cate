...
if (!$row) {
    header('Location: ../admin.php?page=solicitudes');
    exit;
}
...
$rawPhone = preg_replace('/\D+/', '', (string) ($row['phone'] ?? ''));
if ($rawPhone !== '' && strlen($rawPhone) === 8) {
    $rawPhone = '507' . $rawPhone;
}

$whatsAppBaseMessage = "Buenas, " . v3FullName($row)
    . ". Le contactamos con relación a su solicitud "
    . $row['request_number']
    . " en el sistema Verif-Carné.";

$v3WhatsAppUrl = $rawPhone !== ''
    ? 'https://wa.me/' . $rawPhone . '?text=' . rawurlencode($whatsAppBaseMessage)
    : '';

$v3WhatsAppValidated = $rawPhone !== ''
    ? 'https://wa.me/' . $rawPhone . '?text=' . rawurlencode(
        $whatsAppBaseMessage
        . "\n\nSu solicitud ha sido VALIDADA correctamente."
    )
    : '';

$v3WhatsAppObserved = $rawPhone !== ''
    ? 'https://wa.me/' . $rawPhone . '?text=' . rawurlencode(
        $whatsAppBaseMessage
        . "\n\nSu solicitud se encuentra EN OBSERVACIÓN. Por favor, revise las indicaciones proporcionadas."
    )
    : '';

$v3WhatsAppRejected = $rawPhone !== ''
    ? 'https://wa.me/' . $rawPhone . '?text=' . rawurlencode(
        $whatsAppBaseMessage
        . "\n\nSu solicitud ha sido RECHAZADA. Por favor, revise las observaciones y realice una nueva solicitud con la información corregida."
    )
    : '';
...
<section class="v3-expedient-header v3-card">
    ...
    <div class="v3-expedient-header-actions">
        ...
        <?php if ($v3WhatsAppUrl !== ''): ?>
            <a
                class="v3-button whatsapp"
                href="<?= e($v3WhatsAppUrl) ?>"
                target="_blank"
        ...
        <?php else: ?>
            <span class="v3-button disabled" title="La solicitud no tiene un teléfono válido">
                Sin WhatsApp
            </span>
        <?php endif; ?>

        <a class="v3-button secondary" href="admin.php?page=solicitudes">Volver</a>
    </div>
</section>
...
<section class="v3-tabs-card v3-card">
    ...
    <span>Promoción</span>
    <strong>
        <?php if (in_array(strtoupper((string) $row['rank_name']), ['DIRECTOR', 'MNJ'], true)): ?>
            No aplica
        <?php else: ?>
            <?= e(trim((string) ($row['promotion_type'] ?? '') . ' ' . (string) ($row['promotion_number'] ?? ''))) ?>
        <?php endif; ?>
    </strong>
</div>
    ...
    <div class="v3-tab-panel" id="v3-tab-validacion">
        ...
        <div class="v3-review-layout">
            ...
            <form method="post" action="admin_update.php" id="v3ValidationForm" class="v3-review-form">
                ...
                <?php if ($v3WhatsAppUrl !== ''): ?>
                    <div class="v3-whatsapp-panel">
                        <h3>Notificar por WhatsApp</h3>
                        <p>Abra WhatsApp con un mensaje preparado según la decisión.</p>

                        <div class="v3-whatsapp-actions">
                            <a href="<?= e($v3WhatsAppValidated) ?>" target="_blank" rel="noopener noreferrer">
                                Avisar validación
                            </a>
                            <a href="<?= e($v3WhatsAppObserved) ?>" target="_blank" rel="noopener noreferrer">
                                Avisar observación
                            </a>
                            <a href="<?= e($v3WhatsAppRejected) ?>" target="_blank" rel="noopener noreferrer">
                                Avisar rechazo
                            </a>
                        </div>
                    </div>
